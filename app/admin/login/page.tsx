...
export default function AdminLoginPage() {
  ...
  return (
    <div className="min-h-screen bg-dark flex items-center justify-center px-4 relative overflow-hidden">
      <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(ellipse at 50% 50%, #C9A84C, transparent 60%)' }} aria-hidden="true"/>
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="glass rounded-2xl p-8">
          <h2 className="text-xl font-bold text-off-white mb-1">Welcome back</h2>
          <p className="text-muted text-sm mb-7">Sign in to manage your fleet</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div>
              <label htmlFor="password" className="block text-xs font-semibold uppercase tracking-wider text-muted mb-2">Password</label>
            ...
            <button type="submit" disabled={loading}
              className="w-full btn-gold py-3.5 rounded-xl font-bold disabled:opacity-50 shine mt-2">
              {loading ? (
        ...
        <p className="text-center text-muted/40 text-xs mt-6">© {new Date().getFullYear()} TodoRental</p>
      </div>
    </div>
  );
}
